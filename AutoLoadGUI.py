...
def measure(startCurrent, endCurrent, stepSize, delay):
    ...
    measureTime = (stopTime-startTime)/1000
    print("Czas pomiaru: " + str(measureTime) + "ms")
    temp = pd.DataFrame(data, columns = ['prad', 'napiecie', 'power', 'setcurrent'])
    return temp,measureTime

def save_csv(data, title):
    df = pd.DataFrame(data, columns = ['prad', 'napiecie', 'power', 'setcurrent'])
    now = datetime.now()
    date_time = now.strftime("%m_%d_%Y %H-%M-%S")
    df.to_csv("Wyniki/"+ title + date_time + ".csv", index=False)
...
layout2=[[sg.Text('COM:'),sg.Combo(ports,key='com',size=(10,1))],
        ...
        [sg.InputText(size=(18,1),key='-TITLE-', default_text = "Tytul")],
...
def main():
    ...
    while True:
        ...
        if event=='Rozpocznij pomiar':
            ...
            window['M_TIME'].update(f'{measureTime:.0f}s')
        ...
        if event=='Generuj raport PDF':
            ...
            date_time = now.strftime("%m_%d_%Y %H-%M-%S")
            title = values["-TITLE-"]
            pdf.output("PDF/"+title + date_time + '.pdf','F')
            pass
